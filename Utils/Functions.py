...
from Utils.JobReader import skill_lst, skill_dict
...
def evaluate(sampler, environment, data_test, train_samples, epoch, T=16, verbose=False):
    ...
    for x, y in data_test:
        ...
        for t in range(T):
            ...
            # evaluate q range
            '''
            q_salary_range[t].append(q[0])
            q_easy_range[t].append(q[1])
            '''
    ...
    '''
    for t in range(T):
        q_salary_range[t].sort()
        print("q_salary_%s" %(t), q_salary_range[t][0], q_salary_range[t][-1], "average is:", float(sum(q_salary_range[t])/len(q_salary_range[t])))
        print("75%: ", np.percentile(q_salary_range[t], 75), "90%: ", np.percentile(q_salary_range[t], 90), "95%: ", np.percentile(q_salary_range[t], 95))

    for t in range(T):
        q_easy_range[t].sort()
        print("q_easy_%s" %(t), q_easy_range[t][0], q_easy_range[t][-1], "average is:", float(sum(q_easy_range[t])/len(q_easy_range[t])))
        print("75%: ", np.percentile(q_easy_range[t], 25), "90%: ", np.percentile(q_easy_range[t], 10), "95%: ", np.percentile(q_easy_range[t], 5))
    '''
...
def evaluate_case_study(sampler, environment, T, num1, num2):
    ...
    # skill_set = ['java', 'hadoop', 'hive', '性能', '编程', '性能优化', '优化', 'c/c++', '代码', '数据结构', 'coding', 'linux/unix', '并发', '架构', '算法', '统计', 'linux', 'c++', 'c']
    
    skill_set = ['java', 'hive', '算法基础', '性能', '编程', '性能优化', '优化', 'c/c++', 'c++', 'c', '代码', '数据结构', 'coding', 'linux/unix', '并发', '架构', '算法', '算法设计', 'linux']
    ...
    for t in range(T):
        ...
        easy, salary, r = environment.add_skill(s, evaluate=True)
        step_metrics.append((r, easy, salary, q, attention_salary, salary_prototypes, attention_difficulty, difficulty_prototypes, salary_action_value, difficulty_action_value, frequency_set, orginal_frequency_set, loss, encoder_loss, div_loss, freq_loss, chosen_action, difficulty_mapped, (q_salary, q_easy), act_lst, q))
        append_skills.append(skill_lst[s])
