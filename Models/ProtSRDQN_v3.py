...
class ProtSRDQN_dec_v3(object):
    ...
    def _build_network(self):
        ...
        self.deep_encoder_salary_action_dropout2, y_deep_action, _ = self._mlp(y_deep_action4, 512, [256, 256],
                                                        name="%s_deep_encoder_salary_action2" % self.name,
                                                        activation=self.activation, bias=True, sparse_input=False, batch_norm=False)

        # For last layer, we use SoftPlus function
        self.salary_action_value, _ = self._fc(y_deep_action, self.salary_encoder_layers[-1], 1, name="%s_salary_encoder_action" % self.name, l2_reg=0.0, activation = tf.nn.softplus, bias=True, sparse=False) 
        ...
        self.deep_encoder_difficulty_action_dropout2, y_deep_action, _ = self._mlp(y_deep_action4, 512, [256, 256],
                                                        name="%s_deep_encoder_difficulty_action2" % self.name,
                                                        activation=self.activation, bias=True, sparse_input=False, batch_norm=False)

        self.difficulty_action_value, _ = self._fc(y_deep_action, self.difficulty_encoder_layers[-1], 1, name="%s_difficulty_encoder_action" % self.name, l2_reg=0.0, activation = tf.nn.softplus, bias=True, sparse=False) 
