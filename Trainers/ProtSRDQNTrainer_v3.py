        ...
        for it in tqdm(range(n_batch)):
            ...
            if it != 0 and it % verbose_batch == 0:
                ...
                if save_path is None:
                    self.Qnet.save(HOME_PATH + "data/model/%s_ProtSRDQN_dec_v3_Prot_sal_%s_dif_%s_freq_%s_div_%s_enc_%s_bool_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_freq, lambda_div, lambda_enc, diversity1, env_params))
                else:
...
if __name__ == "__main__":
    ...
    '''
    Qa = ProtSRDQN_dec_v3(n_skill, lambda_div= lambda_div, lambda_enc = lambda_enc, lambda_freq = lambda_freq, diversity1 = diversity1, diversity2 = diversity2, verbose=1, learning_rate=0.00025, lambda_d=env_params['lambda_d'], embsize=256, 
               pool_size=env_params['pool_size'], salary_prototype = salary_prototype, difficulty_prototype = difficulty_prototype,
    ...
    Qa_ = ProtSRDQN_dec_v3(n_skill, lambda_div= lambda_div, lambda_enc = lambda_enc, lambda_freq = lambda_freq, diversity1 = diversity1, diversity2 = diversity2, verbose=1, learning_rate=0.00025, lambda_d=env_params['lambda_d'], embsize=256, pool_size=env_params['pool_size'], salary_prototype = salary_prototype, difficulty_prototype = difficulty_prototype, salary_encoder_layers=[256, 256], salary_decoder_layers=[256, 256], dropout_salary=[1.0, 1.0, 1.0], difficulty_encoder_layers=[256, 256], difficulty_decoder_layers=[256, 256], dropout_difficulty=[1.0, 1.0, 1.0], salary_q_value_max = init_salary_q_value, difficulty_q_value_max = init_salary_d_value,activation=tf.nn.leaky_relu, random_seed=2022, l2_reg=0.001, name="target", sess=sess)

    sess.run(tf.global_variables_initializer())

    # ----------------- 模型读取 ---------------------
    Qa.load(HOME_PATH + "data/model/%s_ProtSRDQN_dec_v3_Prot_sal_%s_dif_%s_freq_%s_div_%s_enc_%s_bool_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_freq, lambda_div, lambda_enc, diversity1, str(env_params)))
    ...
    # ----------------- 模型保存 -----------------------
    # Qa.save(HOME_PATH + "data/model/%s_ProtSRDQN_dec_v3_Prot_sal_%s_dif_%s_freq_%s_div_%s_enc_%s_bool_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_freq, lambda_div, lambda_enc, diversity1, env_params))